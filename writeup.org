...
model to be credible, that is to agree with prior knowledge in the field without
lowering performance. Note that maintaining performance is important because
that's what makes the model useful.

It may sound too good to be true that there's no tradoff between performance and
credibility: isn't everything come with a cost? Given we already have an accurate
...
seemingly free lunch property is obtained by leveraging domain knowledge. 
...
risk factors over unknown features when these features are correlated.

* related work

Credibility and interpretability are usually approached through feature
selection, which can be further breakdown into subset selection, shrinkage, and
dimension reduction cite:zou2006adaptive. Subset selection selects subset of
features so that when the model is trained on this subset, the tradeoff between
model simplicity and increase in loss is balanced. This class of methods include
best subset selection and its computationally efficient variant stepwise
selection. Shrinkage methods refer to regularization, which is penalty added on
the size of model parameters. This is the most oftenly used class of methods due
to its non-intrusiveness regarding training pipeline. Dimension reduction
methods try to learn the true dimension of the data so that noise is minimized
and correlation is removed. Example methods include PCA and ICA. In this work,
we focus on regularization methods.
...
which introduces a weight $w_i$ for each feature: $\lambda \sum_{i=1}^n
...
Criteria 1) is acheived by grid searching over the validation set so that
...
group. The metrics used are KL divergence and earth mover's distance. Earth
mover's distance measures the amount of work to turn one distribution to the
other and is symmetric, while KL divergence is asymmetric in its arguments.
...
this translate to model's sensitivity to $\alpha$: small $\alpha$ will let the model
...
** TODO 6_th run (sweep corelation, fix r, fix theta to 1)
    b unregularized
...
** TODO 7_th run (sweep fractional r, fix correlation, fix theta)
...
*** eye penalty label:eye_properties
    ...
    we define eye norm as a an atomic norm $||\cdot||_A$ as introduced in
...
+ \beta doesn't affect the shape of contour, so no need to search over \beta
  ...
  then $q_{\beta_1}(x) = 2 \beta_1 ||(1-r) \odot x||_1 +
  (1-\beta_1) ||r \odot x||_2^2 = \frac{\beta_1^2}{1-\beta_1}$

  let x' = $\frac{\beta_2 (1-\beta_1)}{\beta_1 (1-\beta_2)}$ x

  \begin{align}
  q_{\beta_2}(x') &= 2 \beta_2 ||(1-r) \odot x'||_1 +
  (1-\beta_2) ||r \odot x'||_2^2\\
  &= \frac{2 \beta_2^2 (1-\beta_1)}{\beta_1 (1-\beta_2)} ||(1-r) \odot x||_1 + 
  \frac{\beta_2^2 (1-\beta_1)^2}{\beta_1^2 (1-\beta_2)} ||r \odot x||_2^2\\
  &= \frac{\beta_2^2 (1-\beta_1)}{\beta_1^2 (1-\beta_2)} (2 \beta_1 ||(1-r) \odot x||_1 +
  (1-\beta_1) ||r \odot x||_2^2)\\
...
*** lasso cite:tibshirani1996regression
    ...
    and $S_{\alpha}(x) = x max(0, 1-\frac{\alpha}{|x|})$ (the soft threshold
